...
module EncryptionNewPG
  ...
  def new_pg_encrypt(zookrparams)
    if (zookrparams.class != Hash) || (zookrparams.keys == [])
      return false
    end
    if !zookrparams.has_key?(:key)
      return false
    end
    encrypted_data = Hash[]
    key = zookrparams.delete(:key)
    keys = zookrparams.keys
  ...
  ### can accept multiple key value pairs in the dictionary ###
  def new_pg_decrypt(zookrparams)
    if (zookrparams.class != Hash) || (zookrparams.keys == [])
      return false
    end
    if !zookrparams.has_key?(:key)
  ...
  def new_pg_checksum(zookrparams, key, salt_length = 4)
    if zookrparams.class != Hash
      return false
    end
    if key.empty?
      return false
    ...
    keys.each do |k|
      if str.nil?
        str = zookrparams[k].to_s
        next
      end
      str = str + '|'  + zookrparams[k].to_s
    end
    str = str + '|' + salt
  ...
  ### accepts a hash with key value pairs ###
  ### calculates sha256 checksum of given values ###
  def new_pg_refund_checksum(zookrparams, key, salt_length = 4)
    keys = zookrparams.keys
    keys.each do |k|
      if ! zookrparams[k].empty?
        #if params[k].to_s.include? "REFUND"
        unless zookrparams[k].to_s.include? "|"
            next
        end
        zookrparams[k] = zookrparams[k]
      end
    end
    if zookrparams.class != Hash
      return false
  ...
  def new_pg_verify_checksum(zookrparams, check_sum, key, salt_length = 4)
    if zookrparams.class != Hash
      return false
    end
    if key.empty?
